...
cargo build --release --locked --features stress-tools --bin parqkit --bin parqkit-generate
echo ""

# Check if fixtures exist
if [[ ! -d "$SCRIPT_DIR/fixtures" ]] || [[ -z "$(fd --no-ignore -t f . "$SCRIPT_DIR/fixtures" --max-results 1 2>/dev/null)" ]]; then
...
echo "=============================================="
cargo test --locked --features stress-tools --test stress 2>&1 | tail -20
echo ""
...
echo "=============================================="
cargo test --locked --features stress-tools --test stress -- --ignored --test-threads=1 2>&1 | tail -50
echo ""
